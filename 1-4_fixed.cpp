...
int main(int argc, char *argv[])
{
        ...
        sgm_img.convertTo(sgm_img, -1, 1.0 / i);
        ...
        while(loop_flag){
                cap >> frame;
                if(frame.empty() || cap.get(CV_CAP_PROP_POS_AVI_RATIO) == 1){
                        break;
                }
                ...
                cv::Mat dstImg;
                dstImg.create(dst_img.size(), dst_img.type());
                cv::resize(img, dstImg, dst_img.size());

                dstImg.copyTo(dst_img);
